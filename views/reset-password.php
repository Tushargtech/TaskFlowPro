...
} elseif ($errorCode === 'invalid_link') {
    $errorMessage = 'This reset link is invalid, expired, or already used.';
}

$hasToken = $selector !== '' && $token !== '';
$isTokenValid = false;

if ($hasToken) {
    $userObj = new User($pdo);
    $isTokenValid = $userObj->isPasswordResetTokenValid($selector, $token);
}
...
<html lang="en">
  <head>
    ...
    <title>TaskFlow Pro | Reset Password</title>
  ...
  <body>
          ...
          <div class="col-12 col-sm-10 col-md-8 col-lg-5 col-xl-4">
            <div class="card shadow border-0">
              <div class="card-body p-4 p-sm-5">
                ...
                <p class="text-muted text-center mb-4">Set a new password for your TaskFlow Pro account.</p>
                ...
                <?php if ($isTokenValid): ?>
                <form action="<?php echo APP_BASE; ?>/reset-password" method="post" novalidate>
                ...
                <div class="d-grid gap-2">
                  <a class="btn btn-primary" href="<?php echo APP_BASE; ?>/forgot-password">Request New Reset Link</a>
                  <a class="btn btn-outline-secondary" href="<?php echo APP_BASE; ?>/login">Back to Login</a>
                </div>
                <?php endif; ?>
              </div>
            </div>
    ...
    <script>
      document.addEventListener('DOMContentLoaded', function () {
        const form = document.querySelector('form[action="<?php echo APP_BASE; ?>/reset-password"]');
        if (!form) {
          return;
        }
