...
function handleUsers(string $method, ?int $id, User $userObj): void
{
    ...
    switch ($method) {
        ...
        case Constants::METHOD_POST:
            ...
            // Begin transaction so the user row is only committed after the
            // welcome email is confirmed delivered. If the email send fails the
            // INSERT is rolled back and no orphan record is left in the database.
            $pdo->beginTransaction();

            $created = $userObj->createUser($payload);
            if (!$created) {
                $pdo->rollBack();
                respond(['success' => false, 'message' => 'creation_failed'], 500);
            }

            require_once APP_ROOT . '/libraries/MailHelper.php';
            $fullName = trim((string) ($input['first_name'] ?? '') . ' ' . (string) ($input['last_name'] ?? ''));
            $mailSent = MailHelper::sendPassword($email, $fullName, $plainPassword);

            if (!$mailSent) {
                $pdo->rollBack();
                respond(['success' => false, 'message' => 'mail_failed'], 500);
            }
